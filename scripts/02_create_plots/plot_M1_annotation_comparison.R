...
# plot overlap of protein-coding genes from 2014 - 2017
pc_overlap_upset <- read_tsv('../../tables/annotation_comparison/pc_gene_overlap_2014_to_2017.tsv') %>% 
	rename(`2014`=Year_2014, `2015`=Year_2015, `2016`=Year_2016, `2017`=Year_2017) %>%
	pivot_longer(!c(gene_id, gene_biotype), names_to = 'annotation_year', values_to = 'year') %>% 
#	mutate(annotation_year = factor(annotation_year,
#		levels = c("2014", "2015", "2016", "2017", "2018", "2019", "2020", "2021", "2022", "2023"))) %>%
	drop_na(year) %>% 
	select(-gene_biotype) %>%
	distinct() %>%
	group_by(gene_id) %>%
        summarize(years = list(sort(unique(annotation_year), decreasing = FALSE))) %>%
        ungroup()

upset_plt <- ggplot(pc_overlap_upset, aes(x=years)) + 
	geom_bar() +
	...
	xlab('Annotation Years') +
	ylab('Number of genes') +
	theme(axis.text.y = element_text(size=2),
	      axis.title = element_text(size=5))
ggsave('../../figures/pc_overlap_2014_to_2017_upset_plot.pdf', upset_plt, height = 58, width = 58 , units='mm')

# plot overlap of genes (by position) from 2014 - 2017
gene_by_pos_upset <- read_tsv('../../tables/annotation_comparison/genes_by_positions_2014_to_2017.tsv') %>% 
	rename(`2014`=Year_2014, `2015`=Year_2015, `2016`=Year_2016, `2017`=Year_2017) %>%
	pivot_longer(!c(seqname, start, end), names_to = 'annotation_year', values_to = 'year') %>% 
#	mutate(annotation_year = factor(annotation_year,
#		levels = c("2014", "2015", "2016", "2017", "2018", "2019", "2020", "2021", "2022", "2023"))) %>%
	drop_na(year) %>% 
	distinct() %>%
...
upset_plt <- ggplot(gene_by_pos_upset, aes(x=years)) + 
	geom_bar() +
	geom_text(stat='count', aes(label=after_stat(count)), vjust=-0.5, size = 1) +
	scale_x_upset(n_intersections = 20) +
	theme_combmatrix(
	...
	theme(axis.text.y = element_text(size=2),
	      axis.title = element_text(size=5))
ggsave('../../figures/genes_by_pos_2014_to_2017_upset_plot.pdf', upset_plt, height = 58, width = 58 , units='mm')
